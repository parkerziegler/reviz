...
import type { RevizTextDatum } from '../src/attributes';
...
describe('scales', () => {
  describe('inferXScaleType', () => {
    it('should infer the xScaleType as continuous for numeric axis values', () => {
      const textAttrs = new Array<RevizTextDatum>(50).fill({
        x: `${getRandInt()}`,
        y: `${getRandInt()}`,
        text: `${getRandInt()}`,
      });
    ...
    it('should only read text values from text nodes with the mode y value', () => {
      const axisValues = new Array<RevizTextDatum>(30).fill({
        x: '0',
        y: '0',
        text: `${getRandInt()}`,
      });
      ...
      const labels = new Array<RevizTextDatum>(20).fill({
        x: `${getRandInt()}`,
        y: `${getRandInt()}`,
        text: getRandChar(),
      });
