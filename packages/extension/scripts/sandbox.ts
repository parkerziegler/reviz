...
window.addEventListener("message", (event: MessageEvent<ExecuteMessage>) => {
  ...
  try {
    ...
    // We'll ignore lint warnings for use of new Function and calliing an any
    // typed value; we know we have a function that, when called, returns an
    // SVGSVGElement generated by Observable Plot.
    // eslint-disable-next-line @typescript-eslint/no-implied-eval, @typescript-eslint/no-unsafe-call
    const plot = new Function(`return ${formatExecutableProgram(program)}`)()(
      Plot,
      data
    ) as SVGSVGElement;
