...
const Chart: React.FC<Props> = ({ data }) => {
  // Compute x (date) domain and z (cause) domain.  We use d3's InternSet
  // so we can compute Sets even with Date values.
  ...
  const stackData = Object.entries(deathsByMonth).map(([date, record]) => {
    return {
      date,
